...
Util.setClass = u.sc = function(e, classname) {
	try {
		...
		u.bug("20Exception ("+exception+") in u.setClass, called from: "+arguments.callee.caller);
	}
}
Util.addClass = u.ac = function(e, classname) {
	try {
		...
		u.bug("22Exception ("+exception+") in u.addClass, called from: "+arguments.callee.caller);
	}
}
Util.removeClass = u.rc = function(e, classname) {
	try {
		...
		u.bug("21Exception ("+exception+") in u.removeClass, called from: "+arguments.callee.caller+" with class:" + classname);
	}
}
Util.toggleClass = u.tc = function(e, classname, second_classname) {
	try {
		...
		u.bug("19Exception ("+exception+") in u.toggleClass, called from: "+arguments.callee.caller);
	}
}
Util.hasClass = u.hc = function(e, classname) {
	try {
		...
		u.bug("18Exception ("+exception+") in u.hasClass, called from: "+arguments.callee.caller);
	}
	return false;
}
